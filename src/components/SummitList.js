...
export default function SummitList() {
  ...
  useEffect(() => {
    ...
    const refreshTopPart = (response) => {
      populateFirstRow(response);
      setPartData(prevState => {
        return {
          ...prevState,
          parts: response
    ...
    getSummitList(partData.summit, partData.startDate, setSummitData)

    const refreshTimeout = setInterval(() => {
      if (partData.startDate >= partData.currentDate) {
        getSummitList(partData.summit, new Date().valueOf(), refreshTopPart)
      }
    }, 10000);    
    return () => {
